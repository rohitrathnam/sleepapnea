%% Loading R1 data
load('R1_data.mat');
load('R1_data_header.mat');
tree = xml_read('R1.xml');

%% Segmenting data into 30s epoch
% Signal length = 32520
% Sampling frequency for ECG = 125Hz, for Sp02 = 1Hz
% ECG = 4th row of data
% Sp02 = 1st row

%Extracting ECG and Sp02 data
ECG = record(4, :);
Sp02 = record(1, 32520); %As Sp02 has only 32520 recordings

% Extracting 30 second epochs. 1 second = 125 samples
ECG_epochs = [];
for i = 1:3750:length(ECG)
    ECG_epochs = [ECG_epochs; ECG(i:i+3749), 0];
end

%% =====Getting the hypopnea events ========
Event_data = tree.ScoredEvents.ScoredEvent;
Epoch_indices = [];
Artifact_indices = [];
for i = 1:length(Event_data)
    s = Event_data(i).EventConcept;
    ar = Event_data(i).Notes;
    if strcmp(s,'SDO:HypopneaFinding') == 1
        idx = (Event_data(i).Start);
        epoch_idx = ceil(idx/30) ;
        Epoch_indices = [Epoch_indices; epoch_idx];
        ECG_epochs(epoch_idx, 3751) = 1;
    end
end
...
for i = 1:size(Integrated_Signal,1)
    ...
    PH = zeros(1,60);
    RR = zeros(1,60);
    [pks, loc] = findpeaks(sf(i,:),'MinPeakDistance',60, 'MinPeakHeight', 0.06);
    p_len = length(pks);
    Dist = diff(loc);
...
Feat_mat = [MRR MHR RMSSD SDNN ECG_epochs(:,end)];
Feat_mat_1 = [];
Feat_mat_2 = [];
